...
<!--Html block:/{0}-->";
		...
		private IEnumerable<IHtmlString> IncludeThemeStyles(Site site, string themeName, string baseUri = null)
		{
			...
			if (styles != null && styles.Count() > 0)
			{
				if (site.Mode == ReleaseMode.Debug)
				{
					...
					/* MS: Remove the LESS script insertion
			...
			// MS: Check if there is a point to adding the html string?
			if (!string.IsNullOrEmpty(themeRuleBody))
		...
		public virtual IHtmlString PageLink(object linkText, string urlMapKey, object routeValues, object htmlAttributes)
		{
			...
			if (page != null && page.Route != null && page.Route.LinkTarget.HasValue && !string.IsNullOrEmpty(page.Route.LinkTarget.Value.ToString()))
			{
